 ...
 *
 * A read walks a list of public gateways until one returns the blob, verifies
 * the GCM tag during decryption, and decodes back to the original value.
 *
...
export interface BlobStoreConfig {
  ...
  gateway?:  string | null;     // preferred read gateway (full ".../ipfs/" base)
}
...
/** Fetch an envelope by CID, trying the preferred gateway then public fallbacks. */
async function fetchEnvelope(cid: string, gateway: string | null | undefined): Promise<SealedEnvelope | null> {
  const bases = gateway ? [gateway, ...DEFAULT_GATEWAYS] : DEFAULT_GATEWAYS;
  for (const base of bases) {
    try {
      const res = await fetchWithTimeout(base.replace(/\/?$/, "/") + cid, FETCH_TIMEOUT_MS);
      if (!res.ok) continue;
...
export class EncryptedBlobStore {
  ...
  async read<T = unknown>(label: string, cid: string): Promise<T | null> {
    const envelope = await fetchEnvelope(cid, this.cfg.gateway);
